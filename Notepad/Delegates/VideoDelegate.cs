namespace Notepad
{
	...
	public static class VideoDelegate
	{
		...
		[BindMenuItem(Name = "压缩 SRT (目录)", SplitButton = "videoButton", Toolbar = "toolStrip", AddSeparatorBefore = true)]
		public static void ZipSubtitle()
		{
			Forms.OnClipboardDirectory(dir => {
				dir.ZipSrt();
			});
		}
		[BindMenuItem(Name = "添加 SRT (文件)", SplitButton = "videoButton", Toolbar = "toolStrip", AddSeparatorBefore = true)]
		public static void EmbedSubtitle()
		{
			Forms.OnClipboardFile(f => {
				var dir = Path.Combine(f.GetDirectoryName(), "srt");
				
				dir.CreateDirectoryIfNotExists();
				 
				Process.Start(new ProcessStartInfo {
					FileName = "ffmpeg",
					WorkingDirectory = f.GetDirectoryName(),
					Arguments = string.Format("-i \"{0}\" -vf subtitles=\"{1}\":force_style='FontName=微软雅黑,FontSize=20' -vcodec libx264 -acodec copy -q:v 0 -q:a 0 \"{2}\"", f.GetFileName(), Path.ChangeExtension(f, ".srt").GetFileName(), Path.Combine(dir, f.GetFileName()))
				});
			});
		}
			
		[BindMenuItem(Name = "缩略图 (文件)", SplitButton = "videoButton", Toolbar = "toolStrip")]
		public static void CreateWebp()
		{
			Forms.OnClipboardFile(f => {
				var dir = Path.Combine(f.GetDirectoryName(), "srt");
				...
				Process.Start(new ProcessStartInfo {
					FileName = "ffmpeg",
					WorkingDirectory = f.GetDirectoryName(),
					Arguments = string.Format("-i \"{0}\" -ss 00:00:01.000 -vframes 1 \"{1}\"", f.GetFileName(), Path.ChangeExtension(f, ".webp").GetFileName())
				});
			});
		}
		[BindMenuItem(Name = "缩略图 (HLS)", SplitButton = "videoButton", Toolbar = "toolStrip")]
		public static void CreateWebpFromHLS()
		{
			Forms.OnClipboardText(url => {
				var dir = ".webp".GetDesktopPath();
				...
				Process.Start(new ProcessStartInfo {
					...
					Arguments = string.Format("-y -ss 00:00:01 -t 10 -i \"{0}\" -vf \"scale=iw:-1,fps=1/3\" out%03d.webp", url)
				});
			});
		}
		...
		[BindMenuItem(Name = "下载所有字幕 (文本)", SplitButton = "videoButton", Toolbar = "toolStrip", AddSeparatorBefore = true)]
		public static void DownloadAllSubtitlesFiles()
		{
			Forms.OnClipboardText(url => {
				DownloadSubtitlesFilesInternal(url);
			});
		}
		private async static void DownloadSubtitlesFilesInternal(string str)
		{
			ServicePointManager.SecurityProtocol = SecurityProtocolType.Ssl3 | SecurityProtocolType.Tls12 | SecurityProtocolType.Tls11 | SecurityProtocolType.Tls;
			var client = HttpClients.GetHttpClient();
			
			 
			var hd =	str.GetHtmlDocument();
			
			
			var dstDir = "Videos".GetDesktopPath();
			dstDir.CreateDirectoryIfNotExists();
			var index = 1;
			Task.Run(() => {
				hd.DocumentNode.SelectSingleNode("//*[@id='toc-content']")
				.SelectNodes(".//a[contains(@class,'video-name')]").ForEach(async node => {
				                                                            	
					var id =	node.GetAttribute("data-ga-value");
	
					var name = (index++).ToString().PadLeft(3, '0') + ". " + node.GetInnerText().GetValidFileName();
					name = Regex.Replace(name, "\\s{2,}New$", "");
				
					var srtFile = Path.Combine(dstDir, name + ".srt");
					if (!srtFile.FileExists()) {
						var pieces = node.GetHref().Split('/').Reverse().Take(2).ToArray();
						Debug.WriteLine(pieces[1]);
						var	url = string.Format("https://www.lynda.com/ajax/player/transcript?courseId={0}&videoId={1}", pieces[1], id);
						var content = await client.ReadStringWithCookie(url, "", Cookie);
						srtFile.WriteAllText(content);
					}
				});
		...
		[BindMenuItem(Name = "下载所有视频 (文本)", SplitButton = "videoButton", Toolbar = "toolStrip", AddSeparatorBefore = true)]
		public static void DownloadAllVideosFiles()
		{
			Forms.OnClipboardText(url => {
				DownloadVideosFilesInternal(url);
			});
		}
		private async static void DownloadVideosFilesInternal(string str)
		{
			ServicePointManager.SecurityProtocol = SecurityProtocolType.Ssl3 | SecurityProtocolType.Tls12 | SecurityProtocolType.Tls11 | SecurityProtocolType.Tls;
			var client = HttpClients.GetHttpClient();//HttpClients.GetHttpClientWithProxy();
			...
			var index = 1;
			Task.Run(async () => {
				var nodes = hd.DocumentNode.SelectSingleNode("//*[@id='toc-content']")
					.SelectNodes(".//a[contains(@class,'video-name')]");
				...
				foreach (var node in nodes) {
					...
					name = Regex.Replace(name, "\\s{2,}New$", "");
					var srtFile = Path.Combine(dstDir, name + ".mp4");
					if (!srtFile.FileExists()) {
						var pieces = node.GetHref().Split('/').Reverse().Take(2).ToArray();
						...
						var stream = await videoUrl.Stream();
						using (var outStream = new FileStream(srtFile, FileMode.OpenOrCreate)) {
